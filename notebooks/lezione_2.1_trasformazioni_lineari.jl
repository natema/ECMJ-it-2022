...
# ╔═╡ 0a54d9ae-3d0e-4a4d-bde4-129215bcda55
md"""
...
# ╔═╡ 3c8970a4-ccc9-442b-a0a0-e86ed1f6b5b3
md"""
...
# ╔═╡ a689686e-7bcf-412d-a6b7-090d2fdde733
md"""
# Lezione 2.2 - Trasformazioni lineari
"""

# ╔═╡ 8be7521f-36d3-4d61-b0ed-a9cbab4a73e0
PlutoUI.TableOfContents(aside=true)

# ╔═╡ 78c67dfd-c55e-4e17-ab44-42e9e8ae85c2
md"""
### Scelta della trasformazione

Alla fine di questa lezione potremo scegliere, in modo interattivo, una trasformazione lineare da applicare all'immagine (dunque $A:\mathbb R^2 \rightarrow \mathbb R^2$).
"""

# ╔═╡ 4fbb000b-4756-4554-bd64-b7dc235d39eb
md"""
Al momento, tale matrice è
"""
...
Scegliamo ora l'immagine alla quale applicheremo la trasformazione $A$: 
...
md"""
Mostra griglia? $(@bind show_grid CheckBox(default=true))

Utilizza la funzione `with_gridlines`:
"""
...
function with_gridlines(img::Array{<:Any,2}; n=8)
	...
	# ripetiamo, per creare una linea di 2 pixel di spessore
...
# ╔═╡ 487e72d5-f0fa-4096-bf80-3deca21fea17
md"""
Utilizzando l'array comprehension, per ciascuna coppia $(x,y)$ dove $x$ e $y$ prendono valori in un dato range, determineremo il colore del pixel usando la funzione `trygetpixel` definita di seguito:
"""
...
function trygetpixel(img::AbstractMatrix, x::Float64, y::Float64)
	...
	if 1 < i ≤ rows && 1 < j ≤ cols
		...
		# `zero` restituisce lo "zero" del tipo dato
		# `eltype` restituisce il tipo di un *elemento* dell'oggetto dato
		zero(eltype(img)) 
	end
end

# ╔═╡ 95c94546-0d25-478b-a7a6-4c8c15534bf0
md"""
Utilizzeremo inoltre la funzione `LinRange(start, stop, num)` per creare un insieme di `num` valori equidistribuiti tra `start` e `stop`:
"""
...
let	range = -2:.1:2
	...
	**Ri-valutare questa cella per "resettare" la trasformazione.**
	"""
end
...
[
	if det_A == 0
		...
		# soluzione di $A(x_{in},y_{in})' = (x_{out}, y_{out})'$
...
md"""
Il suo determinante è di fondamentale importanza, poiché stabilisce se la trasformazione scelta è _degenere_, ovvero trasforma preserva l'immagine oppure la _distrugge_ irreparabilmente perdendo una delle due dimensioni (provare per credere). 

Nel caso dell'$A$ scelta sopra, abbiamo
$\det(A)= ad - bc =$ $(a) ``\cdot`` $(d) - $(c) ``\cdot`` $(b) =  $(det(A))

Definiamo di seguito variabili relative al determinante, che useremo più giù: 
"""
