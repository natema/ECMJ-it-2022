...
# ╔═╡ e85f9f68-7aa0-475b-9627-87b365313f6c
md"""
# Lezione 12.0 - Un primo modello climatico
"""

# ╔═╡ 340fc7c4-128f-4476-b445-65005ffa9f5a
md"""
## Il pensiero pre-computazionale
"""
...
Richiamiamo brevemente alcune nozioni di base per coloro che stessero dando un'occhiata al presente notebook senza aver seguito le lezioni precendenti. 

``y=y(t)`` è una funzione del tempo $t$.
Inizialmente $t=0$, $y(0)=y_0$, e scriviamo ``y'`` per ``dy/dt``.

Per esempio $y(t) = \sin(t)$:
"""

# ╔═╡ 2813a93e-db99-44bf-b23d-8d9230c4474f
begin
	t = 0:.01:10
	plot(t, sin, xlabel="\$t\$", label="\$\\sin(t)\$")
end
...
Dopo un corso di base in equazioni differenziali, sappiamo che se ``y' = a``
allora 
...
$$y(t) =\left(- \frac{a}{b}\right)(e^{-bt}-1) + y_0 e^{-bt}.$$

Se $b\neq 0$, allora si trova una situazione di equilibrio risolvendo $y'=0$, ovvero quando $y=a/b$. Tale situazione è anche il limite per $t\rightarrow \infty$, per il quale i termini esponenziali scompaiono.
...
md"""
### ``y'`` lineare in $y$ $+$ "forcing term" in $t$


Se ``y' = a - by + g(t)`` allora

$$y(t) = e^{-bt} \left( y_0 + \int_0^t  e^{bu}(a+g(u)) du \right)$$
...
md"""
### Richiamo: risolvere un ODE ($y'=f(y)$) in Julia

0. Definire $y' = f(y,p,t)$
1. `ODEProblem(  f, f(0), time_span, parameters)`
2. `solve`
3. `plot` etc
"""

# ╔═╡ 0fafec6f-330b-4aa7-abbc-a8be6a370fc8
md"""
Nel caso di $y' = a - by + ct$ con $t\in (0,10)$:
"""
...
begin
	...
	# plot direction field:
	    ...
	    title!("Soluzione di y'(t) = a - by + forcing(c)(t)")
end

# ╔═╡ 7a797fd1-f6a1-4b55-88f2-6b670d0fda04
md"""
!!! warning "Esempio progetto d'esame"
    Quanto segue può essere considerato un buon prototipo di progetto d'esame.
"""

# ╔═╡ aba7fc93-c0ac-4c9e-b975-18145f87707f
md"""
## Background: un po' di fisica del clima

Il più semplice modello climatico può essere concettualizzato come segue

$$\text{quantità di calore} = 
\begin{cases}
+ &\text{\color{orange}{radiazione solare assorbita}} \\
- &\text{\color{red}{radiazione termica emessa}}\\
+ &\text{\color{green}{effetto serra causato dall'uomo}}
\end{cases}$$

dove ciascuno di questi termini va interpretato come una media sull'intero pianeta (dunque una quantità scalare).
"""
...
(Un esempio di $\mathrm{temp}'=\mathrm{constant}$)
"""
...
#### Caso 0: Riscaldando la terra no-stop

Vedremo anzitutto come considerare il primo caso _base_ in cui la radiazione solare viene assorbita continuamente.
"""

# ╔═╡ 087f47b2-8283-4205-88f2-4d5883a340c2
md"""
Considerata la distanza del nostro pianeta dal Sole, la potenza dei raggi solari che colpiscono la terra è pari a
"""

# ╔═╡ 9c89c4e9-65ee-4424-bd74-17168b211797
S = 1368; # insolazione [W/m^2]  (energia per unità di tempo per unità d'area)

# ╔═╡ ca5b41d2-b511-486a-91fe-cceb8f7282c3
md"""
Di tale radiazione, una piccola parte viene riflessa verso lo spazio (per es. dalle nuvole, la superficie coperta da neve e ghiaccio...), mentre la frazione rimanente (pari a $1-\alpha$) viene assorbita:
"""

# ╔═╡ 13984c61-4c34-40db-9043-fcff2721522e
α = 0.3; # albedo, ovvero radiazione solare riflessa [adimensionale]

# ╔═╡ d74936e9-b760-4add-b3e7-46d544064c16
md"""
Mentre in matematica ci possiamo limitare a scrivere un'equazione differenziale, nel mondo fisico ci sono variabili fisiche che devono essere identificate. 

Nel nostro esempio di cottura della Terra, identifichiamo le seguenti quantità:

- Inizio della Rivoluzione Industriale: 1850
- Temperatura media nel 1850: 14.0 °C

- Energia proveniente dal sole (insolazione): $S=1368 \, W/m^2$

- Albedo (radiazione solare riflessa): $\alpha = 0.3$

- Capacità termica dell'oceano e dell'atmosfera: $C=51 \, J/(°C\,m^2)$

#### Ricetta per la cottura della Terra

$C\  \mathrm{temp}'(t) = \frac 14 S(1-α)$ 
e possiamo stimare $S(1-α)/4 =$ $(S*(1-α)/4).
"""

# ╔═╡ 868e19f4-d71e-4222-9bdd-470387991c67
md"""
Alla distanza alla quale ci troviamo dal Sole, possiamo considerare i suoi raggi approssimativamente paralleli; pertanto, l'area ortogonale alla loro direzione che corrisponde a quanta radiazione solare colpisce la terra, è un disco di area
$\pi R^{2}$. 

![](https://www.open.edu/openlearn/ocw/pluginfile.php/101161/mod_oucontent/oucontent/890/639dcd57/ce3f1c3a/s250_3_002i.jpg)

Tutti gli altri fattori che stiamo considerando agiscono sulla _superficie_ terrestre, la cui area è $4\pi R^{2}$ (considerando la Terra sferica); pertanto la radiazione solare assorbita *per unità di superficie* (in media su tutto il pianeta) è ridotta di un fattore 4.

Dunque abbiamo 

$\textcolor{orange}{\text{radiazione solare assorbita} \equiv \frac{S(1-\alpha)}{4}}$
"""

# ╔═╡ 0e4dedc5-0b1e-4e46-b943-284bfb2de57f
absorbed_solar_radiation = S*(1 - α)/4; # [W/m^2]

# ╔═╡ 045c562c-935f-44d2-b4c0-f90cac68188b
md"""
Dichiariamo le variabili discusse in Julia:
"""

# ╔═╡ 91329865-f593-4388-a5d0-01af4be6e01d
begin
	C = 51.; # capacità termica degli oceani e dell'atmosfera [J/m^2/°C]
	temp₀ = 14.0 # temperatura preindustriale [°C]
end

# ╔═╡ 86b6a052-cbc5-4fd2-8501-8edbdcafddf5
md"""
Possiamo risolvere questo primo problema:  
"""
...
begin
	plot(solve(p1),       legend = false, 
		 background_color_inside = :black,
		                  xlabel = "Anni dal $(1850)",
	                      ylabel = "Temperatura °C")
	hline!( [temp₀,temp₀] ,c=:white,ls=:dash)
	annotate!( 80, 25+temp₀, text("Temperatura preindustriale = $(temp₀)°C",color=:white))
	title!("Radiazione solare assorbita (senza altri fattori)")
end

# ╔═╡ ad1e294e-ad8a-48f9-b924-2474cce16aaf
md"""
Il _contenuto termico_ $C\, \mathrm{temp}$ è determinato dalla temperatura $\mathrm{temp}$ e dalla capacità del sistema climatico. 
Il nostro interesse è rivolto alla temperatura dell'atmosfera, la quale ha una capacità termica assai ridotta, ma che d'altro canto è fortemente legata a quella degli oceani, i quali hanno una capacità termica assai più grande. 
"""

# ╔═╡ 2086e386-194b-4863-a9ee-178002925d42
md"""
Il cambiamento nel tempo della **quantità di calore** è dato dalla semplice formula $\frac{d(CT)}{dt}$. 
Poiché la capacità termica del mare cambia in modo trascurabile rispetto alla temperatura, possiamo riscrivere tale formula in funzione del solo cambiamento della temperatura nel tempo: 

$\color{red}{\text{cambiamento in contenuto calorico} =\; C \frac{d\mathrm{temp}}{dt}}$
"""
...
md"""
La _radiazione termica_ $\mathcal{G}(T)$ che emana dal nostro pianeta rappresenta l'effetto combinato del **ciclo di feedback negativo** che ne attenua il riscaldamento (cfr. [raffreddamento per radiazione](https://en.wikipedia.org/wiki/Radiative_cooling)) e il **ciclo di feedback positivo** che lo amplifica (per es. [l'effetto del vapore acqueo](https://en.wikipedia.org/wiki/Earth%27s_energy_budget#Heating/cooling_rate_analysis)). 

![](https://upload.wikimedia.org/wikipedia/it/thumb/6/6f/Sun_climate_system_it.svg/1280px-Sun_climate_system_it.svg.png)

Poiché la complessità di tale effetti fisici esula dal contenuto del corso, in questo notebook li *linearizziamo* al fine di combinarne nel modello i contributi positivi e negativi. 

Assumeremo che il mondo precedente la rivoluzione industriale fosse in stato di equilibrio termico. In particolare, la temperatura di equilibrio sarà dunque considerata essere quella preindustriale. 

Partanto assumiamo che

$\mathrm{temp}'(t) = B(\mathrm{temp}(0)-\mathrm{temp}(t))$ per un valore opportuno di $B$.
(Il segno negativo davanti $\mathrm{temp}(t)$ indica che l'equazione è volta a riattrarre il sistema verso lo stato d'equilibrio.)
"""

# ╔═╡ edafca88-dd4a-4fe4-8ffc-bb50f7bd561d
md"""
Per ora consideriamo il seguente valore di $B$:
"""

# ╔═╡ 9b97e41b-76ff-4cc2-90c1-446d59c4ece7
B = 1.3; # parametro di feedback climatico [W/(m^2 °C)],

# ╔═╡ 0b39c1bf-6c18-4eaf-b82c-618e01d9ca97
md"""
Per quanto riguarda la temperatura iniziale, la variamo interattivamente per osservarne l'effetto:
"""
...
begin
	plot(solve(p2),       legend = false, 
		 background_color_inside = :black,
		                  xlabel = "anni dall'inizio",
	                      ylabel = "Temperatura °C",
	                      ylim = (0, 30))
	hline!( [temp₀,temp₀] ,c=:white,ls=:dash)
	annotate!( 80, temp₀, text("Temperatura preindustriale = $(temp₀)°C",:bottom,color=:white))
	title!("Modello del bilancio d'energia (Terra sana)")
end

# ╔═╡ 20d6c513-2ca6-4dea-9092-156e2805d467
md"""
## Effetto serra causato dall'uomo 🏭

L'[effetto serra](https://it.wikipedia.org/wiki/Effetto_serra) è _empiricamente_ noto essere una funzione logaritmica della concentrazione dell'anidride carbonica: 

$\color{green}{\text{effetto serra causato dall'uomo}\; = {\mbox {(forcing\_coef)}} \ln \left( \frac{[\text{CO}₂]}{[\text{CO}₂]_{\text{PreIndust}}} \right),}$

Come tale quantità varia nel tempo dipende dal comportamento umano. 
Notare che il tempo non compare come variabile nell'equazione precedente. 
"""

# ╔═╡ 692a6928-c7a2-4b61-a794-16bef5d4e919
md"Per i parametri finora considerati, abbiamo i valori seguanti:"

# ╔═╡ 6d1058bf-8a05-4b8e-835a-de9e95f567c4
forcing_coef = 5.0; # CO2 "forcing coefficient" [W/m^2]

# ╔═╡ 35184e6f-ac3e-480b-92e9-ed8baaaa833b
CO₂_PreIndust = 280.; # concentrazione preindustriale di CO2 [parti per milione, ppm]
...
# ╔═╡ 318e6223-bebb-4d37-9185-d7706c33e0c0
md"""
Possiamo sperimentare diverse ipotesi per quanto riguarda l'andamento della quantità di diossido di carbonio $CO_2$: 
"""
...
begin
	...
	title!("Livello di CO₂ negli anni")
end

# ╔═╡ c6f8dcf6-950d-48ff-b040-55bdd347d74b
md"""
### Rilevamenti dal vulcano Mauna Loa ![Mauna Loa Volcano](https://i.pinimg.com/originals/df/1a/e7/df1ae72cfd5e6d0d535c0ec99e708f6f.jpg)
"""

# ╔═╡ cd1dcbc4-2273-4eda-85d9-9af4fd71b3c1
md"""
I dati sono disponibili all'indirizzo [https://www.ncei.noaa.gov/pub/data/paleo/icecore/antarctica/law/law2006.txt](https://www.ncei.noaa.gov/pub/data/paleo/icecore/antarctica/law/law2006.txt).
"""
...
begin
	...
	CO2_historical_data = CSV.read(download(CO2_historical_data_url), DataFrame, header=55, datarow=58, );
...
md"""
I dati che ci interessano, vale a dire il livello di CO₂, sono forniti dalla quinta colonna. 

Notare che sono presenti dei **missing data** ($-99.99$).
"""

# ╔═╡ 2bc4c596-b313-49f6-84b5-5d53f2baf0e9
validrowsmask = CO2_historical_data[:, "     CO2"] .> 0 
# gli spazi nell'header andrebbero puliti..
...
md"""
Usaremo il fitting cubico per fare un confronto con le temperature storiche. 
"""

# ╔═╡ acc662c8-9fb9-4e12-88f1-a4fc7866abc2
md"""
## Radiazione termica ed effetto serra  ♨ 🏭

Combiniamo ora i sistemi precedenti. 
"""
...
begin
	...
	hline!( [temp₀,temp₀] ,c=:white,ls=:dash)
	annotate!( 80, temp₀, text("Temperatura preindustriale = $(temp₀)°C",:bottom,color=:white))
	title!("Modello con CO₂")
end

# ╔═╡ d34653a6-b252-4bcd-8a67-a1e1ed85499b
md"""
Infine rilassiamo le assunzioni su $B$ e $C$, esplorando l'effetto del loro valore in modo interattivo. Definiamo dunque il nuovo sistema con parametri `BB` e `CC` che andiamo a controllare a breve, sotto il grafico della soluzione. 
"""

# ╔═╡ a5ab7ef1-a723-4135-8729-83c79087a403
md"""
Acquisendo da internet ([dati NASA](https://data.giss.nasa.gov/gistemp/graphs/graph_data/Global_Mean_Estimates_based_on_Land_and_Ocean_Data/graph.txt)) i valori storici relativi alla variazione della temperatura media, possiamo verificare l'accuratezza del nostro modello:
"""
...
begin
	...
	T_df = CSV.read(download(T_url), DataFrame, header=3, datarow=5, delim=" ", ignorerepeated=true);
    # T_df = T_df[:,[1,6]]
end

# ╔═╡ 3304174c-289d-47c5-b5ef-161b11e515eb
md"""
Feedback climatico BB = $(@bind BB Slider(0:.1:4, show_value=true, default=B))

Capacità termica dell'oceano CC =$(@bind CC Slider(10:.1:200, show_value=true, default=C))
"""
...
begin
	plot(years,solp4.(years.-1850),lw=2,label="Temperature predette dal modello", legend=:topleft)
	xlabel!("Anno")
	ylabel!("Temp °C")
	
	plot!( T_df[:,1], T_df[:,2] .+ 14.15, color=:black, label="Osservazioni della NASA", legend=:topleft)
end

# ╔═╡ 28acb5a4-2a5f-49c5-9c78-deb40fdeed36
md""" 
##  Previsioni del riscaldamento globale 
"""

# ╔═╡ 13b003d2-1fd4-4a4a-960c-4a1d9b673dc6
md"""
Possiamo distinguire due futuri ipotetici molto diversi: 

1. Un futuro caratterizzato da **basse emissioni**, in cui quest'ultime decrescono in modo che la concentrazione di CO₂ resti inferiore a 500 ppm entro il 2100 (tale scenario è tecnicamente noto come "RCP2.6");
2. Un futuro caratterizzato da **alte emissioni**, in cui quest'ultime continuano ad aumentare e la concentrazione di CO₂ sale fino a 1200 ppm (scenario technicamente noto come "RCP8.5")
"""
...
md"""
Nello scenario a basse emissioni, l'incremento di temperatura si mantiene inferiore a $\Delta T = 2$ Cᵒ entro il 2100, mentre nello scenario ad alte emissioni le temperature salgono fino a 3.5 Cᵒ al di sopra dei livelli preindustriali. 
"""
