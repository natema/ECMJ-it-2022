...
# ╔═╡ 42cc92fe-4f8d-4bf0-a14c-97dc47982c19
md"""
...
begin
	io_fl = IOBuffer(HTTP.get("https://www.pilucrescenzi.it/gm/graphs/florence.lg").body);
	g_fl = loadgraph(io_fl, "graph", LGFormat());
	deg_fl = degree_centrality(g_fl,normalize=false);
	sum(deg_fl)/nv(g_fl)
end
...
begin
	io_jp = IOBuffer(HTTP.get("https://www.pilucrescenzi.it/gm/graphs/japan.lg").body);
	g_jp = loadgraph(io_jp, "graph", LGFormat());
	dist_jp = gdistances(g_jp,5)
end
...
begin
	far_jp = zeros(Int64, nv(g_jp))
	for u in 1:nv(g_jp)
		far_jp[u] = sum(gdistances(g_jp,u))
	end
	far_jp
end
...
let
	io_imdb = IOBuffer(HTTP.get("https://www.pilucrescenzi.it/gm/graphs/imdb.lg").body);
	g_imdb = loadgraph(io_imdb, "graph", LGFormat());
	println("Distance between RB and JV: ", gdistances(g_imdb, 3)[8])
	rem_vertex!(g_imdb, 7)
	println("Distance between RB and JV: ", gdistances(g_imdb, 3)[7])
end
...
let
	io_imdb = IOBuffer(HTTP.get("https://www.pilucrescenzi.it/gm/graphs/imdb.lg").body);
	g_imdb = loadgraph(io_imdb, "graph", LGFormat());
	println("Distance between GC and BA: ", gdistances(g_imdb, 1)[4])
	rem_vertex!(g_imdb, 2)
	println("Distance between GC and BA: ", gdistances(g_imdb, 1)[4])
end
...
let
	...
	to_be_removed = 0
	max_not_reachable = 0
	for a in articulation(g_imdb)
		io_imdb_a = IOBuffer(HTTP.get("https://www.pilucrescenzi.it/gm/graphs/imdb.lg").body);
		g_imdb_a = loadgraph(io_imdb_a, "graph", LGFormat());
		not_reachable_with_a = 0
		for u in 1:nv(g_imdb_a)
			dist = gdistances(g_imdb_a, u)
			for v in 1:nv(g_imdb_a)
				if (dist[v] >= nv(g_imdb_a))
					not_reachable_with_a = not_reachable_with_a+1
				end
			end
		end
		rem_vertex!(g_imdb_a, a)
		not_reachable_without_a = 0
		for u in 1:nv(g_imdb_a)
			dist = gdistances(g_imdb_a, u)
			for v in 1:nv(g_imdb_a)
				if (dist[v] >= nv(g_imdb_a))
					not_reachable_without_a = not_reachable_without_a+1
				end
			end
		end
	...
	rem_vertex!(g_imdb, to_be_removed)
	println("Communities after removing node $to_be_removed: ", connected_components(g_imdb))
end
...
let
	io_karate = IOBuffer(HTTP.get("https://www.pilucrescenzi.it/gm/graphs/karate.lg").body);
	g_karate = loadgraph(io_karate, "graph", LGFormat());
	println("Neighborhood of node 1: ", all_neighbors(g_karate, 1))
end
